...
extern void rust_fiat_curve25519_carry_square_CryptOpt(uint64_t* out, const uint64_t* arg1);

// Global variables for BINSEC to track
uint64_t secret_input[4];
...
int main() {
    ...
    rust_fiat_curve25519_carry_square_CryptOpt(public_output, secret_input);
