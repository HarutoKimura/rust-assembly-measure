#include <stdint.h>
#include <string.h>
#include <stdlib.h>

// External assembly function
extern void open_ssl_curve25519_fe51_square_CryptOpt(uint64_t* out, const uint64_t* arg1);

// Global variables for BINSEC to track
uint64_t secret_input[4];
uint64_t public_output[4];

int main() {
    // Initialize secret input with symbolic values
    // BINSEC will treat this as secret when marked in the config
    for (int i = 0; i < 4; i++) {
        secret_input[i] = 0x3333333333333333ULL * (i + 1);
    }
    
    // Call the assembly function
    open_ssl_curve25519_fe51_square_CryptOpt(public_output, secret_input);
    
    // Return value based on output to prevent optimization
    return (int)(public_output[0] & 0xFF);
}
