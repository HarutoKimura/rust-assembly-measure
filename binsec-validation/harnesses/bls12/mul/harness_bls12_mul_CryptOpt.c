...
extern void bls12_mul_CryptOpt(uint64_t* out, const uint64_t* arg1, const uint64_t* arg2);

// Global variables for BINSEC to track
uint64_t secret_input1[6];
uint64_t secret_input2[6];
uint64_t public_output[6];
...
int main() {
    ...
    bls12_mul_CryptOpt(public_output, secret_input1, secret_input2);
