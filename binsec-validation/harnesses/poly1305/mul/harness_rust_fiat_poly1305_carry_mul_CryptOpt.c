...
extern void rust_fiat_poly1305_carry_mul_CryptOpt(uint64_t* out, const uint64_t* arg1, const uint64_t* arg2);

// Global variables for BINSEC to track
uint64_t secret_input1[3];
uint64_t secret_input2[3];
...
int main() {
    ...
    rust_fiat_poly1305_carry_mul_CryptOpt(public_output, secret_input1, secret_input2);
