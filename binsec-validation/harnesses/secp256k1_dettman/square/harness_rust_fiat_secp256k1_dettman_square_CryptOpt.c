...
extern void rust_fiat_secp256k1_dettman_square_CryptOpt(uint64_t* out, const uint64_t* arg1);

// Global variables for BINSEC to track
uint64_t secret_input[5];
...
int main() {
    ...
    rust_fiat_secp256k1_dettman_square_CryptOpt(public_output, secret_input);
